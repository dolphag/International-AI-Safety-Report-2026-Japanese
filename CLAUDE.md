...
5. **References章（p.157-219、参考文献リスト）はHTML化しない**。目次ページに「参考文献は原文PDFを
   参照」の旨を記載し、原文PDF（`international-ai-safety-report-2026_1.pdf`）へのリンクを置く。
...
- `translations/ja/` — 章ごとの日本語訳（Markdown or 中間テキスト）を格納する想定
...
14. `13_references.txt` — References（翻訳・HTML化ともに対象外。目次ページから原文PDFへリンクする）
